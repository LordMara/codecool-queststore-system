CREATE TABLE IF NOT EXISTS `quests` (
	...
	FOREIGN KEY(`quest_category_id`) REFERENCES `quests_category`(`quest_category_id`)
);
INSERT INTO `quests` VALUES (1,'Exploring a dungeon','Finishing a Teamwork week',100.0,1),
 (2,'Solving the magic puzzle','Finishing an SI assignment',100.0,1),
 (3,'Slaying a dragon',' Passing a Checkpoint',500.0,1),
 (4,'Spot trap','Spot a major mistake in the assignment',50.0,2),
 (5,'Taming a pet','Doing a demo about a pet project',100.0,2),
 (6,'Recruiting some n00bs','Taking part in the student screening process',100.0,2),
 (7,'Forging weapons','Organizing a workshop for other students',400.0,2),
 (8,'Master the mornings','Attend 1 months without being late',300.0,2),
 (9,'Fast as an unicorn','deliver 4 consecutive SI week assignments on time',500.0,2),
 (10,'Achiever','set up a SMART goal accepted by a mentor, then achieve it',1000.0,2),
 (11,'Fortune','students choose the best project of the week. Selected team scores',500.0,2),
 (12,'Creating an enchanted scroll','Creating extra material for the current TW/SI topic (should be revised by mentors)',500.0,2),
 (13,'Enter the arena','Do a presentation on a meet-up',500.0,2);
CREATE TABLE IF NOT EXISTS `quests_category` (
	`quest_category_id`	INTEGER PRIMARY KEY AUTOINCREMENT,
	`name`	TEXT
);
INSERT INTO `quests_category` VALUES (1,'Basic'),
 (2,'Extra');
